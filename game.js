...
function NewQuestion(data, length) {
	...
	if (length < data.results.length) {
		...
		fetch(
			`https://api.mymemory.translated.net/get?q=${quizQueston}&langpair=en|fa`
		)
			.then(res => res.json())
			.then((output) => {
				console.log(output.responseData.translatedText),
				quizQueston = output.responseData.translatedText
